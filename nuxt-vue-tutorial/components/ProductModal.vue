        ...
        <div class="w-full relative flex items-center bg-white px-4 pt-14 pb-8 overflow-hidden shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-8">
          <div class="w-full grid grid-cols-1 gap-y-8 gap-x-6 items-start sm:grid-cols-12 lg:gap-x-8">
            <div class="aspect-w-2 aspect-h-3 rounded-lg bg-gray-100 overflow-hidden sm:col-span-4 lg:col-span-5">
              <img :src="product.image" alt="" class="object-center object-cover" />
            </div>
            <div class="sm:col-span-8 lg:col-span-7 flex flex-col justify-between h-full">
              <button @click="closeModalClick" type="button" class="flex justify-end text-gray-400 hover:text-gray-500">
                <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
              <h2 class="text-2xl font-extrabold text-gray-900">{{ product.name }}</h2>
              <div class="my-7">
                <p class="text-2xl text-gray-900">{{ product.price.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' }) }}</p>
                <span class="mt-1 pl-1 text-sm text-gray-500 border-solid border-2 border-indigo-100 bg-slate-200">-{{ product.discount }}%</span>
              </div>
              <p class="w-full text-sm text-gray-500">{{ product.description }}</p>
...
export default Vue.extend({
  ...
  methods: {
    ...mapActions({
      deleteProduct: ACTIONS.DELETE_PRODUCT,
    }),
    ...mapMutations({
